...
//        Post newPost = new Post(0, 1, 250.00f, "chairs", Post.Type.OTHER);
...
//        Assertions.assertEquals(id, gottenRequest.getId());
    ...
    @Test
    @Order(3)
    void get_all_post_requests_test(){
        List<Post> requestList =  postDAO.getAllPost();
...
//            randomDescr += alphabet[randNum];
        ...
        Post request = postDAO.getPostById(1);
...
//        Assertions.assertEquals(randomDescr, updatedRequest.getDescription());
    ...
    void append_photo_test() {
        ...
        postDAO.appendPhotoToPost(request, bytes);
        Post request2 = postDAO.getPostById(1);
        System.out.println(request2);
        byte[] bytes2 =  request2.getImage();
        Assertions.assertEquals(50, bytes2.length);
    }
    @Test
    @Order(7)
    void delete_post_test(){
        boolean returnValue = postDAO.deletePostById(2);
